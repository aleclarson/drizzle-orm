...
export function tests() {
	describe('common', () => {
		...
		test('insert into ... select with generated column', async (ctx) => {
			...
			const result1 = await db.insert(users2).select(db.select({ name: users1.name }).from(users1)).returning();
			const result2 = await db.insert(users2).overridingSystemValue().select(
		...
		test('insert multiple rows into table with generated identity column', async (ctx) => {
			...
			let result = await db.insert(identityColumnsTable).overridingSystemValue().values([
				{ name: 'John' },
				{ name: 'Jane' },
				{ name: 'Bob' },
			]).returning();
